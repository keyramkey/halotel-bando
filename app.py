...
SMS_SENDER_ID = os.environ.get("SMS_SENDER_ID", "SULE")  # max 11 chars
...
def forgot_password():
    """Hatua 1: weka namba → tuma OTP kwa SMS otomatiki."""
    if get_current_user():
        return redirect(url_for("home"))
    ...
    if request.method == "POST":
        identity = request.form.get("identity", "").strip()
        if not identity:
            flash("Weka username au namba ya simu.", "error")
        ...
        ok, msg = send_otp_sms(user.phone, otp)
        session["reset_user_id"] = user.id

        if ok:
            flash("OTP imetumwa kwa SMS kwenye namba yako. Ingiza hapa chini.", "success")
        else:
            # Bado tunaendelea — OTP ipo; onyesha hitilafu ya SMS
            flash(f"OTP imetengenezwa lakini SMS imeshindwa: {msg}. Jaribu tena au wasiliana admin.", "error")
...
def reset_password():
    """Hatua 2: ingiza OTP + password mpya."""
    if get_current_user():
        return redirect(url_for("home"))
    ...
    if request.method == "POST":
        ...
        if not otp or not password:
            flash("Jaza OTP na password mpya.", "error")
            return redirect(url_for("reset_password"))
        ...
        if not user or not user.reset_otp:
            flash("Omba OTP upya kwanza.", "error")
            return redirect(url_for("forgot_password"))
        ...
        if user.reset_otp_expires and user.reset_otp_expires < datetime.utcnow():
            user.reset_otp = ""
            user.reset_otp_expires = None
            db.session.commit()
            flash("OTP imeisha muda (dakika 15). Omba upya.", "error")
            return redirect(url_for("forgot_password"))
